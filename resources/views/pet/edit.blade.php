...
<div class="h-full w-full flex justify-center items-center " style="background-image: url(/assets/background-try.jpg); background-size:cover; background-repeat: repeat-y">
    ...
    <div class="flex flex-col items-center shadow-lg shadow-gray-900 bg-white rounded-md p-12">
        <form class="flex flex-col items-center rounded-md gap-2" method="post" action="{{route('pet.update', ['id' => $data['id']])}}" enctype="multipart/form-data">
            @csrf
            <div class="flex flex-col">
                <label for="pet_name">Pet Name</label>
                <input value="{{$data['pet_name']}}" class="w-full border border-gray-300 px-3 py-2 rounded-lg shadow-sm" type="text" name="pet_name">
            </div>
            <div class="flex flex-col">
                <label for="age">Age</label>
                <input value="{{$data['age']}}" class="w-full border border-gray-300 px-3 py-2 rounded-lg shadow-sm" type="text" name="age">
            </div>
            <div class="flex flex-col">
                <label for="breed">Breed</label>
                <input value="{{$data['breed']}}" class="w-full border border-gray-300 px-3 py-2 rounded-lg shadow-sm" type="text" name="breed">
            </div>
            <div class="flex flex-col w-full">
                <label for="breed">Sex</label>
                <select class="form-control m-bot15 w-full" name="sex" value="{{$data['sex']}}">
            ...
            <br>
            <button type="submit" class="bg-sky-300 w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-tiny text-white">Update pet</button>
        </form>
    </div>
</div>
