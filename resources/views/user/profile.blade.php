            ...
            <div>
                ...
                <form class="flex flex-col rounded-md" method="post" action="{{route('user.update', ['id' => $user['id']])}}" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <input value="{{$user['id']}}" class="w-full border border-gray-300 px-3 py-2 rounded-lg shadow-sm" type="text" name="id">
                    </div>
                    <div>
                        <label for="first_name">First Name</label>
                        <input value="{{$user['first_name']}}" class="w-full border border-gray-300 px-3 py-2 rounded-lg shadow-sm" type="text" name="first_name">
                    </div>
                    <div>
                        <label for="last_name">Last Name</label>
                        <input value="{{$user['last_name']}}" class="w-full border border-gray-300 px-3 py-2 rounded-lg shadow-sm" type="text" name="last_name">
                    </div>

                    <div>
                        <label for="email">Email</label>
                        <input disabled value="{{$user['email']}}" class="w-full border border-gray-300 px-3 py-2 rounded-lg shadow-sm" type="text" name="email">
                    </div>
